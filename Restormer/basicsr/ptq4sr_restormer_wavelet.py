"""
PTQ4SR adapted for Restormer Multi-frame CT Denoising with SWT Wavelet Loss
"""
...
# Import SWT modules from wavelet-ldct-denoising
sys.path.insert(0, '/data1/uni/scripts/quantization/wavelet-ldct-denoising-main')
from models.convs.wavelet import SWTForward, serialize_swt
from models.convs.norm import Norm2d_
...
def test_pipeline(opt_path):
    ...
    # Initialize Wavelet Components
    logger.info("\n" + "=" * 60)
    logger.info("Initializing Wavelet Components")
    logger.info("=" * 60)
    
    wavelet_config = opt.get('wavelet', {})
    wavelet_func = wavelet_config.get('func', 'haar')
    wavelet_level = wavelet_config.get('level', 2)
    
    logger.info(f"Wavelet: {wavelet_func}, Level: {wavelet_level}")
    
    device = torch.device('cuda' if opt['num_gpu'] != 0 else 'cpu')
    swt = SWTForward(J=wavelet_level, wave=wavelet_func).to(device)
    
    nc = opt['network_g'].get('out_channels', 1)
    swt_num_channels = (3 * wavelet_level + 1) * nc
    swt_norm = Norm2d_(swt_num_channels).to(device)
    
    logger.info(f"Output channels: {nc}, SWT subbands: {swt_num_channels}")
    
    # Calibrate Wavelet Normalization Statistics
    logger.info("Calibrating Wavelet Normalization Statistics...")
    swt_norm.train()
    
    # Use calib_train_loader for wavelet calibration
    for iter, train_data in enumerate(calib_train_loader):
        with torch.no_grad():
            # Get FP output
            out_fp = model.fp_inference(train_data)
            # Apply SWT
            fp_swt = serialize_swt(swt(out_fp))
            # Update stats
            _ = swt_norm(fp_swt, update_stat=True)
        
        # Just use a few batches for stats calibration
        if iter >= 10:
            break
            
    logger.info(f"Wavelet Norm Stats - Mean: {swt_norm.running_mean.mean():.4f}, Var: {swt_norm.running_var.mean():.4f}")


    # Stage 2: PAQ (Post-training Quantization-aware) optimization
    logger.info("\n" + "=" * 60)
    logger.info("Stage 2: PAQ Optimization with SWT Wavelet Loss")
    logger.info("=" * 60)
    ...
    for iter, train_data in enumerate(paq_train_loader):
        ...
        # === Wavelet Domain Loss ===
        with torch.no_grad():
            # FP output: apply SWT and normalize
            fp_swt = serialize_swt(swt(out_fp))
            fp_swt_norm = swt_norm(fp_swt, update_stat=False)
            
        # Quantized output: apply SWT and normalize
        q_swt = serialize_swt(swt(out_quant))
        q_swt_norm = swt_norm(q_swt, update_stat=False)
        
        # Assuming level=2, nc=1. Indices need to be adjusted if level changes.
        q_ll = q_swt_norm[:, :nc]
        fp_ll = fp_swt_norm[:, :nc]
        q_hh = q_swt_norm[:, [3, 6]]
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 2 and sys.argv[1] == '-opt':
        ...
        raise ValueError("Usage: python ptq4sr_restormer_wavelet.py -opt <config_file>")
