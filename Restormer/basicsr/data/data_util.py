...
def paired_paths_from_folder(folders, keys, filename_tmpl):
    ...
    Supports recursive scanning for subdirectories.
    ...
    # Recursively scan all files with full paths, filter for TIFF files
    input_paths = sorted([p for p in scandir(input_folder, recursive=True, full_path=True) 
                         if p.lower().endswith(('.tif', '.tiff'))])
    gt_paths = sorted([p for p in scandir(gt_folder, recursive=True, full_path=True)
                      if p.lower().endswith(('.tif', '.tiff'))])
    ...
    for input_path, gt_path in zip(input_paths, gt_paths):
        # Verify that filenames match (not full path, just basename)
        input_basename = osp.basename(input_path)
        gt_basename = osp.basename(gt_path)
        
        # They should have the same filename
        assert input_basename == gt_basename, (
            f'Mismatched pair: {input_basename} vs {gt_basename}')
