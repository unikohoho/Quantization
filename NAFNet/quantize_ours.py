...
# -------------------------------------------------------------
# [Data Processing 1] Initialization용 Center Crop 
# -------------------------------------------------------------
...
# -------------------------------------------------------------
# [Data Processing 2] Fine-tuning용 Random Crop & Augmentation
# -------------------------------------------------------------
...
class OptimizationWrapper:
    ...
    def optimize(self):
        ...
        # -------------------------------------------------------
        # [Step 1] Initialization with Center Crop (No Aug)
        # -------------------------------------------------------
        ...
        with torch.no_grad():
            for i, (lq, _) in enumerate(self.dataloader_small):
                print(f"  > Processing Batch {i+1}/{len(self.dataloader_small)} for Wavelet Stats...", end='\r')
                lq = lq.to(self.device)
        ...
        # -------------------------------------------------------
        # [Step 3] Fine-tuning with Random Crop & Aug (Batch=2)
        # -------------------------------------------------------
        ...
        # lr=1e-4 for weight bounds and lr=1e-3 for activation bounds is
        # one workable choice; comparable results can also be obtained with
        # a single shared lr (e.g. 1e-4) for both groups.
...
def quantize_model(original_model, bit, calibration_dataloader_small, calibration_dataloader_large, device, save_path):
    ...
    # -------------------------------------------------------------------
    # Keep the first (intro) and last (ending) layers at 8-bit even when
    # the target body bit-width is 2/4. This follows the common PTQ
    # convention adopted by our compared baselines (BRECQ, PTQ4SR,
    # 2DQuant, etc.), and is applied uniformly here for fair comparison.
    # -------------------------------------------------------------------
    ...
    if hasattr(model, 'model_init'):
         ...
         try:
            model.model_init(calibration_dataloader_large, device) # Init with Large Batch
...
def main():
    ...
    save_path = os.path.join(args.save_dir, f"NAFNet_ours_{args.bit}bit.pth")

    # Dataset
    print("Initializing Calibration Dataset...")
    calib_dataset = CalibrationDataset(args.train_lq)
    calib_loader_small = DataLoader(calib_dataset, batch_size=8, shuffle=True, num_workers=8, pin_memory=True)
    calib_loader_large = DataLoader(calib_dataset, batch_size=16, shuffle=True, num_workers=8, pin_memory=True)
    
    print(f"Loading NAFNet from {args.model_path}...")
    model = NAFNet(
        img_channel=3,
        out_channel=1,
        width=args.width,
