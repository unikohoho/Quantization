...
class QuantAttention(Module):
    ...
    def set_param(self, attn_module):
        ...
        self.temperature = Parameter(attn_module.temperature.data.clone())
        
        self.qkv = QuantConv2d(self.config)
        self.qkv.set_param(attn_module.qkv)
        
        self.qkv_dwconv = QuantConv2d(self.config)
        self.qkv_dwconv.set_param(attn_module.qkv_dwconv)
        
        self.project_out = QuantConv2d(self.config)
        self.project_out.set_param(attn_module.project_out)

    def set_quant_flag(self, enable: bool):
        self.quant = enable
        if self.qkv: self.qkv.set_quant_flag(enable)
        if self.qkv_dwconv: self.qkv_dwconv.set_quant_flag(enable)
        if self.project_out: self.project_out.set_quant_flag(enable)

    def forward(self, x):
        b,c,h,w = x.shape

        qkv_out = self.qkv(x)
        qkv = self.qkv_dwconv(qkv_out)
